...
class Accounts{
    ...
    function doLoginUser(){
        ...
        $userLog = $this->db->query("SELECT id, name, password, status, currency FROM `".$config['db']['prefix']."Users` WHERE (email = '".$this->db->escapeStr($_REQUEST['login_email'])."'  OR name = '".$this->db->escapeStr($_REQUEST['login_email'])."' ) LIMIT 1");
        if (count($userLog) ==  1) {
            ...
            if ($check) {
                ...
                $_SESSION['currency']=$userLog[0]['currency'];
        ...
        if(count($warning)==0){
            ...
            if($bt[0]['fees'][0]['pairs']!=''){
                ...
                $sql = "INSERT into `".$config['db']['prefix']."Users` (`name`,`email`,`password`,`currency`,`bfxapikey`,`bfxapisec`,`status` )
					 ...
					 '".$this->db->escapeStr($_REQUEST['new_currency'])."', '".$this->db->escapeStr($_REQUEST['new_bfxKey'])."', '".$this->db->escapeStr($_REQUEST['new_bfxSec'])."', '".$this->db->escapeStr($_REQUEST['new_actType'])."' )";
                $newUser = $this->db->iquery($sql);

                if($newUser['id']!=0){
                    //  Set default settings for the account //
                    $sql = "INSERT into `".$config['db']['prefix']."Vars` (`id`,`minlendrate`,`spreadlend`,`gapBottom`,`gapTop`,`sevenDayMin`,`thirtyDayMin`,`highholdlimit`,`highholdamt` )
						 VALUES
						 ( '".$newUser['id']."', '0.0650', '3', '25000', '100000', '0.1000', '0.1500', '0.3500', '0' )";
    ...
    function getAccount(){
        ...
        if($userInfo[0]['bfxapikey'] !='' && $userInfo[0]['bfxapisec'] !=''){
            ...
            $this->currency = $userInfo[0]['currency'];
    ...
    function displayDetailsTableRow(){
		...
		<tr class="bigrow '.( ($this->sts == 2 || $this->sts == 8 ) ? 'danger':'').'" id="userRow_'.$this->userid.'">
				...
				<td class="mid">'.$gen->moneyFormat($thirtydayRet['intTotal'], $this->currency).'<div><small>'.$gen->percentFormat($thirtydayRet['avgInt']).'</small></div></td>
				<td class="mid">'.$gen->moneyFormat($fullRet['intTotal'], $this->currency).'<div><small>'.$gen->percentFormat($fullRet['avgInt']).'</small></div></td>
			</tr>
					  ...
					  <div id="collapseListGroup'.$this->userid.'" class="panel-collapse collapse" role="tabpanel" aria-labelledby="collapseListGroupHeading'.$this->userid.'" aria-expanded="false" style="height: 0px;">
						<table class="table table-striped table-bordered" style="padding: 0px;margin: 0px;">
							...
							<tr style="font-size: 10px;">
								...
								<th class="mid" style="width: 110px;">
									<div style="height:25px;padding-top:8px;" aria-label="Help" class="pull-right"  data-toggle="popover" data-placement="right" title="Minimum for 7 Day Loans" data-content="Lowest Daily Interest Rate at which your loans will be offered for 7 days instead of 2.  Set this to insure your loans earn you a higher interest rate for longer periods durring flash lending runs.  If you want all loans to stay on a 2 day cycle, set this to 0.">
									  <span class="glyphicon glyphicon-question-sign" aria-hidden="true"></span>
									</div>
									Minimum<br>for 7 Day
								</th>
								<th class="mid" style="width: 110px;">
							...
							<tr>
								...
								<td>
									<div class="input-group">
										<input type="text" name="sevenDayMin" value="'.number_format($this->bfx->actSettings['sevenDayMin'], 4).'" class="form-control autoPercent">
										<span class="input-group-addon">%</span>
									</div>
								</td>
								<td>
								...
								<td>
									<div class="input-group">
										<span class="input-group-addon">'.($this->currency=='USD' ? '$' : substr($this->currency,0,1)).'</span>
							...
							<tr>
								<td colspan="2">
									<div class="input-group">
										<span class="input-group-addon">'.($this->currency=='USD' ? '$' : substr($this->currency,0,1)).'</span>
										<input type="text" name="gapBottom" value="'.number_format($this->bfx->actSettings['gapBottom'], 2).'" class="form-control autoCurrency">
									</div>
								</td>
								<td colspan="2">
									<div class="input-group">
										<span class="input-group-addon">'.($this->currency=='USD' ? '$' : substr($this->currency,0,1)).'</span>
										<input type="text" name="gapTop" value="'.number_format($this->bfx->actSettings['gapTop'], 2).'" class="form-control autoCurrency">
									</div>
								</td>
								<td>
									<div class="input-group">
    ...
    function updateSettings(){
        ...
        $gapBottom = preg_replace('/[^0-9.]/', '', $_REQUEST['gapBottom']);
        $gapTop = preg_replace('/[^0-9.]/', '', $_REQUEST['gapTop']);
        $sevenDayMin = preg_replace('/[^0-9.]/', '', $_REQUEST['sevenDayMin']);
        ...
        $sql = "UPDATE `".$config['db']['prefix']."Vars` SET  minlendrate = '".$this->db->escapeStr($minlendrate)."', spreadlend = '".$this->db->escapeStr($spreadlend)."',
				gapBottom = '".$this->db->escapeStr($gapBottom)."', gapTop = '".$this->db->escapeStr($gapTop)."',
				thirtyDayMin = '".$this->db->escapeStr($thirtyDayMin)."',
				sevenDayMin = '".$this->db->escapeStr($sevenDayMin)."',

...
class Bitfinex{
    ...
    public function __construct($userid, $apiKey, $apiSec, $nonce=0) {
        ...
        $sql = "SELECT currency from `".$config['db']['prefix']."Users` WHERE id = '".$userid."' LIMIT 1";
        $res = $this->db->query($sql);
        $this->currency = strtolower($res[0]['currency']);
    ...
    function bitfinex_query($method, array $req = array()) {
        ...
        $reqData = base64_encode(json_encode($req));
    ...
    function bitfinex_updateHistory($numDays=30,  $sinceLast=0){
        global $config;
        ...
        if($sinceLast==0){
            ...
            if($sinceLast <=1){
                ...
                $sinceLast = strtotime("-30 days");
            }
        }
        $ledgerDetails = array('currency' => strtoupper($this->currency), 'since' => (string)$sinceLast, 'until' => (string)($sinceLast + (86400 * $numDays)), 'wallet' => 'deposit');
        ...
        foreach($ledgerHistory as $l){
            ...
            if( in_array(strtolower($l['description']), array('swap payment on wallet deposit', 'margin funding payment on wallet deposit')) ){
                $sql = "INSERT into `".$config['db']['prefix']."Tracking` (`user_id`, `trans_id`, `date`, `dep_balance`,`swap_payment`,`average_return`) VALUES
					('".$this->db->escapeStr($this->userid)."', '".$this->db->escapeStr($l['timestamp'])."', '".$this->db->escapeStr(date('Y-m-d', $l['timestamp']))."', '".$this->db->escapeStr($l['balance'])."','".$this->db->escapeStr($l['amount'])."','".$this->db->escapeStr(round((($l['amount'] / $l['balance']) * 100),8))."')";
...
//            }
    ...
    function bitfinex_getLastPrice($currency = null){
        if(!$currency){
    ...
    function bitfinex_updateMyLends(){
        ...
        //		4. Creates ( $spreadlend ) Loan Offers spread evenly between the rates at which ( $gapBottom ) and ( $gapTop ) are reached
        ...
        // Update last price of this currency (used to determine minimum loan which is $50 worth of whatever currency it is)
        $this->lastPrice = $this->bitfinex_getLastPrice();
    ...
    function bitfinex_getMyLendRates(){
        ...
        $loans = array();
        $undercut = 0.00001 * 365; // undercut offer by this much (daily rate converted to annual)
        // how much we got?
        ...
        if($this->available >= $limit){
            ...
            if($splitAvailable > $limit){
                ...
                if($numSplits >= 1){
                    $gapClimb = ( ($this->actSettings['gapTop'] - $this->actSettings['gapBottom'])/($numSplits-1));
                    $nextlend = $this->actSettings['gapBottom'];
                    ...
                    foreach($this->lendbook as $l){
                        while( ($l['totamt']>=$nextlend) && ($a <= $numSplits) ){
                            ...
                            $loans[$a]['rate'] = ( ($l['rate'] - $undercut) >= $minLendRateAnnual ? ($l['rate'] - $undercut) : $minLendRateAnnual ) ;
                            ...
                            $loans[$a]['time'] = 2;
                            if(($this->actSettings['sevenDayMin']>0)&&($l['rate'] >= ($this->actSettings['sevenDayMin'] * 365))){
                                $loans[$a]['time'] = 7;
                            }
                            if(($this->actSettings['thirtyDayMin']>0)&&($l['rate'] >= ($this->actSettings['thirtyDayMin'] * 365))){
                                $loans[$a]['time'] = 30;
                            }
                            $nextlend += $gapClimb;
                            $a++;
    ...
    function bitfinex_getAccountSettings(){
        ...
        if($userSettings[0]['minlendrate'] !=''){
            ...
            $this->actSettings['gapBottom'] = $userSettings[0]['gapBottom'];
            $this->actSettings['gapTop'] = $userSettings[0]['gapTop'];
            $this->actSettings['sevenDayMin'] = $userSettings[0]['sevenDayMin'];
